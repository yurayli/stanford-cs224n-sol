...
def question_1h_sanity_check():
    ...
    print ("-"*80)
    sentence_length = 10
    inpt = torch.randn(sentence_length, BATCH_SIZE, EMBED_SIZE, dtype=torch.float32)
...
def question_1i_sanity_check():
    ...
    max_word_length = 21
    kernel_size = 5
    inpt = torch.randn(BATCH_SIZE, char_embed_size, max_word_length, dtype=torch.float32)
